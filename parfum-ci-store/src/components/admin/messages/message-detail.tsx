import {
  ...
  NoteForm,
  StatusActionButtons,
} from "@/components/admin/messages/message-actions";
...
function whatsappHref(number: string | null, orderNumber?: string | null) {
  ...
  const text = encodeURIComponent(
    `Bonjour, nous vous contactons au sujet de votre message envoyé à PerfumeCI${orderNumber ? ` pour la commande ${orderNumber}` : ""}.`,
  );
        ...
        <section className="rounded-lg border bg-surface p-5">
          ...
          <dl className="mt-5 grid gap-3 text-sm md:grid-cols-2">
            ...
            <Info
              ...
              value={message.sourceReference ?? message.externalHandle ?? "-"}
            />
          </dl>
        </section>
            ...
            {message.statusHistory.map((entry) => (
              <article key={entry.id} className="rounded-md border p-3 text-sm">
                <p className="font-medium">
                  {entry.fromStatus ? `${messageStatusLabel(entry.fromStatus)} → ` : ""}
                  {messageStatusLabel(entry.toStatus)}
                </p>
            ...
            {message.notes.map((note) => (
              <article key={note.id} className="rounded-md border p-3 text-sm">
                ...
                <p className="mt-2 text-muted-foreground">
                  {new Date(note.createdAt).toLocaleString("fr-FR")} · {note.actorName ?? "Staff"}
                </p>
              </article>
            ))}
          </div>
      ...
      <aside className="grid content-start gap-4">
        <section className="rounded-lg border bg-surface p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {message.phone ? (
              <a href={`tel:${message.phone}`} className={buttonVariants({ variant: "outline" })}>
                Appeler
              </a>
            ) : null}
            {wa ? (
            ...
            {mail ? (
              <a href={mail} className={buttonVariants({ variant: "outline" })}>
                Répondre e-mail
              </a>
            ) : null}
          </div>
          <p className="mt-3 text-xs text-muted-foreground">
            Ouvrir un lien de réponse ne change pas le statut du message.
          </p>
        </section>
        ...
        <section className="rounded-lg border bg-surface p-4 text-sm">
          ...
          <dl className="mt-3 grid gap-2">
            {Object.entries(message.attribution).map(([key, value]) => (
              <Info key={key} label={key} value={value ?? "-"} />
            ))}
          </dl>
          <p className="mt-3 text-muted-foreground">
            Connecté en tant que {getRoleLabel(staff.role)}.
          </p>
        </section>
      </aside>
    </div>
