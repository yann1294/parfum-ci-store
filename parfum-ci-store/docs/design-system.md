...
- On `/panier`, the action hierarchy is: primary `Commander via WhatsApp` when configured, secondary `Finaliser la commande en ligne`, then `Continuer mes achats`. WhatsApp copy must explain that availability, delivery fees, and payment are confirmed with the team and must not imply a confirmed order.
- Checkout uses a mobile-first one-column form with a compact order summary. Desktop may use a two-column layout with the summary beside the form. Required customer, delivery, payment, and terms fields are controlled inputs with visible labels, field errors, disabled-state help, and no placeholder-only labels.
- Checkout payment method labels and instructions come from managed settings. Do not hard-code merchant numbers, beneficiaries, or manual-payment copy inside checkout components.
- With the Phase 14 migration applied, checkout displays the authoritative quote and confirmation/tracking display the stored integer fee, including a legitimate zero fee. `À confirmer` remains only a pre-Phase-14 compatibility fallback.
- Order confirmation details require checkout-flow proof and should be visually restrained: order number, French status labels, masked contact data, safe item summary, payment instructions from settings, and support actions. Direct visits use a generic recovery state.
- Order tracking uses a simple form for order number and telephone, followed by a limited timeline. Tracking results must not put customer data in the URL and must preserve entered values after a failed lookup.
...
- Admin inventory lists follow the same narrow-container strategy: wide operational tables with internal horizontal scrolling when space allows, and stacked cards on narrow desktop/tablet/mobile. Inventory cards must preserve SKU, product, variant, publication state, variant state, initialization, stock on hand, reserved, available, threshold, stock status, and action access.
- Inventory operation dialogs must show the current stock summary, predicted result, reserved-invariant warning, explicit confirmation, and safe error/success feedback. `ADJUSTMENT` must expose increase/decrease controls instead of signed numeric input.
- Admin order lists follow the same narrow-container strategy as inventory: wide operational tables with internal horizontal scrolling when space allows, and stacked cards on narrow desktop/tablet/mobile. List views mask contact data.
- Admin order detail screens show immutable customer and item snapshots, timeline, payment history, notification history, audit summary, inventory lifecycle and append-only notes as restrained operational cards. Valid actions are explicit buttons, not a generic status dropdown.
- Cancellation, delivery, return and payment confirmation dialogs must show the predicted business effect, require confirmation, and keep order status separate from payment status.
- Admin notification center screens use the same dense table plus narrow-card strategy as orders and inventory. Broad list views show masked recipients, compact status badges, attempt counts, safe error codes, and explicit retry/cancel actions. Detail views show structured summaries and attempt history, never raw payload JSON or rendered email HTML by default.
- Admin message inbox screens use the same dense table plus mobile-card strategy. List views show masked contact details and bounded excerpts; detail views use restrained operational cards for customer content, context, status timeline, assignment and internal notes.
