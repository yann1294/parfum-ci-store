-- Phase 8: transactional guest-order creation and atomic inventory reservation.
...
alter table public.customers
  drop constraint if exists customers_normalized_phone_format,
  add constraint customers_normalized_phone_format check (
    normalized_phone is null or normalized_phone ~ '^\+225[0-9]{10}$'
  );
...
alter table public.order_items
  drop constraint if exists order_items_currency_xof,
  add constraint order_items_currency_xof check (currency = 'XOF'),
  drop constraint if exists order_items_size_positive,
  add constraint order_items_size_positive check (size_ml is null or size_ml > 0);
...
alter table public.store_settings
  add column if not exists enabled_payment_methods public.payment_method[] not null
    default array['CASH_ON_DELIVERY']::public.payment_method[],
  add column if not exists enabled_delivery_methods text[] not null
    default array['HOME_DELIVERY']::text[];

alter table public.store_settings
  drop constraint if exists store_settings_enabled_delivery_methods_known,
  add constraint store_settings_enabled_delivery_methods_known check (
    enabled_delivery_methods <@ array['HOME_DELIVERY', 'PICKUP']::text[]
  ),
  drop constraint if exists store_settings_enabled_delivery_methods_not_empty,
  add constraint store_settings_enabled_delivery_methods_not_empty check (
    cardinality(enabled_delivery_methods) > 0
  ),
  drop constraint if exists store_settings_enabled_payment_methods_not_empty,
  add constraint store_settings_enabled_payment_methods_not_empty check (
    cardinality(enabled_payment_methods) > 0
  );

create table if not exists app_private.guest_order_idempotency (
  ...
  constraint guest_order_idempotency_operation_not_blank check (length(btrim(operation)) > 0),
  constraint guest_order_idempotency_key_not_blank check (length(btrim(idempotency_key)) >= 32),
  constraint guest_order_idempotency_fingerprint_format check (request_fingerprint ~ '^[a-f0-9]{64}$'),
  constraint guest_order_idempotency_status_known check (status in ('PROCESSING', 'COMPLETED'))
);

create unique index if not exists guest_order_idempotency_operation_key_idx
  on app_private.guest_order_idempotency(operation, idempotency_key);

create index if not exists guest_order_idempotency_expires_at_idx
  on app_private.guest_order_idempotency(expires_at);

grant usage on schema app_private to service_role;
revoke all on app_private.guest_order_idempotency from public, anon, authenticated;
grant select, insert, update on app_private.guest_order_idempotency to service_role;

create or replace function app_private.guest_order_confirmation(target_order_id uuid)
returns jsonb
language sql
stable
security definer
...
as $$
  select jsonb_build_object(
    ...
    'subtotalXof', orders.subtotal_xof,
    'deliveryFeeXof', orders.delivery_fee_xof,
    'totalXof', orders.total_xof,
  ...
  left join lateral (
    select jsonb_agg(
      jsonb_build_object(
        ...
        'variantLabel', order_items.variant_name,
        'quantity', order_items.quantity,
        'unitPriceXof', order_items.unit_price_xof,
        'lineTotalXof', order_items.total_price_xof
      )
...
revoke all on function app_private.guest_order_confirmation(uuid) from public, anon, authenticated;

create or replace function app_private.generate_order_number()
returns text
language plpgsql
volatile
security definer
...
begin
  loop
    attempt := attempt + 1;
    candidate := 'CMD-' || to_char(timezone('utc', now()), 'YYYY') || '-' ||
    ...
    if attempt >= 10 then
      raise exception 'ORDER_NUMBER_GENERATION_FAILED' using errcode = 'P0001';
    end if;
  end loop;
end;
$$;

revoke all on function app_private.generate_order_number() from public, anon, authenticated;
...
create or replace function public.create_guest_order_server(request jsonb)
returns jsonb
language sql
volatile
security definer
set search_path = ''
as $$
  select app_private.create_guest_order(request);
$$;

revoke all on function public.create_guest_order_server(jsonb) from public;
revoke all on function public.create_guest_order_server(jsonb) from anon;
revoke all on function public.create_guest_order_server(jsonb) from authenticated;
grant execute on function public.create_guest_order_server(jsonb) to service_role;

comment on function app_private.create_guest_order(jsonb) is
  'Phase 8 private guest-order transaction. Locks variants deterministically, validates authoritative catalogue/inventory state, creates order records, reserves inventory, writes ledger/status/audit/outbox rows, and records idempotency atomically.';

comment on function public.create_guest_order_server(jsonb) is
  'Service-role-only wrapper for app_private.create_guest_order because app_private is not exposed through Supabase REST.';
