-- Phase 9 repair: remove fragile customer ON CONFLICT inference from guest-order creation.
...
declare
  ...
  v_normalized_phone text := btrim(coalesce(customer_payload->>'phone', ''));
  v_normalized_whatsapp text := nullif(btrim(coalesce(customer_payload->>'whatsapp', '')), '');
  v_customer_email extensions.citext := nullif(lower(btrim(coalesce(customer_payload->>'email', ''))), '')::extensions.citext;
...
begin
  ...
  select count(*), count(distinct phase8_order_lines.variant_id)
  ...
  if exists (select 1 from phase8_order_lines where phase8_order_lines.quantity < 1 or phase8_order_lines.quantity > 20) then
    raise exception 'ORDER_INVALID_QUANTITY' using errcode = 'P0001';
  end if;

  if v_normalized_phone !~ '^\+225[0-9]{10}$' then
    raise exception 'ORDER_INVALID_PHONE' using errcode = 'P0001';
  end if;

  if v_normalized_whatsapp is not null and v_normalized_whatsapp !~ '^\+225[0-9]{10}$' then
    raise exception 'ORDER_INVALID_PHONE' using errcode = 'P0001';
  end if;
  ...
  where store_settings.id is true
  for share;
  ...
  left join lateral (
    select product_images.object_path
  ...
  if exists (
    ...
    where phase8_locked_variants.actual_product_id <> phase8_locked_variants.submitted_product_id
  ) then
    raise exception 'ORDER_ITEM_UNAVAILABLE' using errcode = 'P0001';
  end if;
  ...
  if exists (
    select 1
    from phase8_locked_variants
    where phase8_locked_variants.product_status <> 'ACTIVE'::public.product_status
      or length(btrim(phase8_locked_variants.product_name)) = 0
      or length(btrim(coalesce(phase8_locked_variants.product_description, ''))) = 0
      or not app_private.product_meets_active_requirements(phase8_locked_variants.actual_product_id)
      or phase8_locked_variants.variant_active is not true
      or phase8_locked_variants.price_xof <= 0
  ) then
    raise exception 'ORDER_ITEM_UNAVAILABLE' using errcode = 'P0001';
  end if;
  ...
  if exists (
    select 1
    from phase8_locked_variants
    where phase8_locked_variants.inventory_initialized_at is null
  ) then
  ...
  if exists (
    select 1
    from phase8_locked_variants
    where phase8_locked_variants.stock_on_hand - phase8_locked_variants.reserved_quantity < phase8_locked_variants.quantity
  ) then
    raise exception 'ORDER_INSUFFICIENT_STOCK' using errcode = 'P0001';
  end if;

  select coalesce(sum(phase8_locked_variants.price_xof * phase8_locked_variants.quantity), 0)
  into subtotal_value
  from phase8_locked_variants;
  ...
  perform pg_advisory_xact_lock(hashtextextended('guest_customer:' || v_normalized_phone, 0));

  select customers.id
  into customer_id_value
  from public.customers
  where customers.normalized_phone = v_normalized_phone
  for update;

  if found then
    update public.customers
    set
      email = coalesce(v_customer_email, customers.email),
      whatsapp = coalesce(v_normalized_whatsapp, customers.whatsapp)
    ...
    returning customers.id into customer_id_value;
  end if;
  ...
  returning orders.id into order_id_value;

  for line_record in select * from phase8_locked_variants order by phase8_locked_variants.variant_id loop
    ...
    where product_variants.id = line_record.variant_id;
  ...
  where guest_order_idempotency.operation = 'guest_order_create'
    and guest_order_idempotency.idempotency_key = v_idempotency_key;
...
comment on function app_private.create_guest_order(jsonb) is
  'Phase 8 private guest-order transaction. Phase 9 repair removes fragile customer ON CONFLICT inference, using a phone-scoped advisory lock plus explicit customer lookup/update/insert while preserving atomic reservation behaviour.';
