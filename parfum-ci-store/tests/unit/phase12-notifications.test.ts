...
describe("Phase 12 notification configuration and providers", () => {
  ...
  it("selects the development provider outside production and redacts recipients", async () => {
    ...
    const { getNotificationConfig, resetNotificationConfigForTests } = await import("@/lib/notifications/config");
    ...
    expect(info).toHaveBeenCalledWith("NOTIFICATION_DEV_EMAIL", expect.objectContaining({
  ...
  it("fails production configuration without Resend settings", async () => {
    ...
    const { getNotificationConfig, resetNotificationConfigForTests } = await import("@/lib/notifications/config");
    resetNotificationConfigForTests();
    expect(() => getNotificationConfig()).toThrow("Production notification delivery must use the Resend provider");
  });
  ...
  it("maps Resend success and retryable failures without exposing secrets", async () => {
    ...
    const fetchMock = vi.fn()
      ...
      .mockResolvedValueOnce({ ok: false, status: 429, statusText: "Too Many Requests", json: async () => ({ name: "rate_limit", message: "try later" }) });
    ...
    await expect(provider.sendEmail(input)).resolves.toMatchObject({ ok: true, providerMessageId: "email_123" });
    await expect(provider.sendEmail(input)).resolves.toMatchObject({ ok: false, retryable: true, errorCode: "rate_limit", errorMessage: "try later" });
  });
});

describe("Phase 12 notification admin contracts", () => {
  it("normalizes filters and masks recipients", async () => {
    const { maskRecipient, normalizeNotificationFilters } = await import("@/lib/notifications/admin");
    expect(maskRecipient("awa@example.com")).toBe("a***@example.com");
    expect(normalizeNotificationFilters({ status: "DROP", channel: "EMAIL", sort: "next_attempt_asc", page: "-5" })).toMatchObject({
      status: "ALL",
      channel: "EMAIL",
  ...
  it("ships claim, result, cancel and low-stock state database migration", () => {
    const sql = readFileSync("supabase/migrations/20260804120000_phase12_notification_processing.sql", "utf8");
    ...
    expect(sql).toContain("revoke insert, update, delete on public.notifications from anon, authenticated");
